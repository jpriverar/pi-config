...
- `regex:(?:^|[.!?]\s+|\n)\s{0,}(?:[-*]|\d+[.)])?\s{0,}(?:use|adopt|choose|implement|deploy|switch to)\s+\w+|\b(?:will|would)\s+(?:solve|fix)\s+this\b|\bhere(?:'s| is)\s+(?:the\s+)?(?:implementation|code|solution)\b|```|\b(?:const|function|class)\s+\w+`
