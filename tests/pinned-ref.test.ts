...
test("pi update preserves an immutable configured Git tag", async () => {
  ...
  try {
    ...
    git(source, ["add", "."]);
    git(source, ["commit", "-q", "-m", "Fixture v0.0.1"]);
    git(source, ["tag", "-a", "v0.0.1", "-m", "Fixture v0.0.1"]);
    ...
    git(source, ["commit", "-q", "-m", "Fixture v0.0.2"]);
    git(source, ["tag", "-a", "v0.0.2", "-m", "Fixture v0.0.2"]);
    git(root, ["clone", "-q", "--bare", source, bare]);
    git(bare, ["config", "daemon.export", "ok"]);

    const expected = git(bare, ["rev-parse", "v0.0.1^{commit}"]);
    const newer = git(bare, ["rev-parse", "v0.0.2^{commit}"]);
    assert.notEqual(expected, newer);
    ...
    const spec = `git:git://127.0.0.1:${port}/fixtures/fixture.git@v0.0.1`;
    const environment = {
      ...process.env,
